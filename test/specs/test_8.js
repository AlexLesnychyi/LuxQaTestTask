...
beforeEach(async () => {
   ...
   async function verifyCartIsEmpty() {
    ...
    it('Valid Checkout', async () => {
        ...
        //add Item to Cart and verify it
        // let item = "sauce-labs-backpack"
        await addToCart(items[2])
        await verifyCart(items[2])

        await $('#checkout').click()
        await expect(await browser.getUrl()).toBe("https://www.saucedemo.com/checkout-step-one.html")
        
        await inputData("John", "Smith", " 12345")
        await $('#continue').click()
        await expect(await browser.getUrl()).toBe("https://www.saucedemo.com/checkout-step-two.html")
        await verifyTotalPrice()

        await $('#finish').click()
        await expect(await browser.getUrl()).toBe("https://www.saucedemo.com/checkout-complete.html")
        await expect($("//h2[text()='Thank you for your order!']")).toBeExisting()

        await $('#back-to-products').click()
        await verifyCartIsEmpty()
        await $('#continue-shopping').click()
        await verifyProductsExist(items)
